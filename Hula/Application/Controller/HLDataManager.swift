...
//                         ["icon" : "icon_cat_other" , "name" : "OTHERS"]]
    ...
    func httpGet(urlstr:String, taskCallback: @escaping (Bool, Any?) -> ()) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            //print(request)
            //print(response)
            let parsedResponse = HLDataManager.parseJSONResponse(data: data, response: response, error: error)
            //print(json)
    ...
    static func parseJSONResponse(data: Data?, response: URLResponse?, error: Error?) -> (ok: Bool, json: Any?) {
        if let error = error {
            print(error)
            return (false, nil)
        }
        ...
        if let httpStatus = response as? HTTPURLResponse, httpStatus.statusCode < 200 || httpStatus.statusCode >= 300 {
            print("HTTP status code was \(httpStatus.statusCode)")
