...
4. **LinkedIn** — rotate the app secret and OAuth state if they ever appeared in git history. Replace `LinkedInClientId`, `LinkedInClientSecret`, `LinkedInState`, `LinkedInRedirectUrl`, `LIAppId`, and the matching `li...` URL scheme in `Hula/Info.plist` from a private build configuration.
