...
4. **LinkedIn** — rotate the client secret and state if they were ever committed; set `LIAppId`, `LIAppSecret`, `LIState`, `LIRedirectURL`, and the `li<app id>` URL scheme in `Hula/Info.plist` from your private app configuration.
