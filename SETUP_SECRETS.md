...
5. **LinkedIn** — rotate the app client secret/state if they ever appeared in git history. Inject `LIAppId`, `LIClientSecret`, `LIState`, `LIRedirectURL`, and the matching `li<app id>` URL scheme from private build configuration before enabling LinkedIn validation.
