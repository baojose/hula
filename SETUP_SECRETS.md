...
2. **LinkedIn** — rotate the OAuth client secret and state if they ever appeared in history. Provide private values for `LIAppId`, `LIAppSecret`, `LIState`, and `LIRedirectURL` in `Info.plist` before enabling LinkedIn validation.
3. **Firebase / Google** — in Google Cloud Console, **restrict** the iOS API key (bundle ID); consider **rotating** keys. Download a fresh `GoogleService-Info.plist` and keep it **out of public repos** if possible.
